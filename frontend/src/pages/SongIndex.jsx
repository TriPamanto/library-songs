...
const SongIndex = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this song?')) {
      axios.delete(`http://127.0.0.1:8000/api/songs/${id}`).then(response => {
  ...
  return (
    <div className="container-fluid">
      ...
      <Link to="/song/create" className="btn btn-primary mb-3">Create</Link>
                ...
                {songs.map((song, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      ...
                      <button onClick={() => handleDelete(song.id)} className="btn btn-danger -ml-1">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
